...
Wird die Stichprobe anhand dem Alter, genauer anhand dem Median, unterteilt ($Median=24$ siehe \ref{label.stichprobe}) so ergibt sich eine leichte Korrelation von $r=.134$ ($p=.01$) zwischen $MMI_{angepasst}$ und $SPANE-N$ derjenigen Studierenden, die vom Alter her jünger als 24 Jahre alt sind $N=611$ (siehe Tabelle \ref{table.ergebnis.alter}). Wiederum ergibt sich für die Variable $MMI$ ein ähnlicher Zusammenhang von $r=.127$ ($p=.01$). \\
Bei dnejenigen Studierenden, die älter oder gleich 24 Jahre alt sind, lässt sich kein Zusammenhang finden $N=695$.
