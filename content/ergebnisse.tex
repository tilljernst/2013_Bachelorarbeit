...
\end{table}
Der Wert für die Unterteilung in starke Medien-Multiasking Nutzer lag beim $MMI_{ext}$ für $HMMs >= 2.35$ ($M + SD$ resp. 1.35 + 1.00) und für schwache Multitasker bei $LMMs <= .35$ ($M - SD$ resp. 1.35 - 1.00). Analog dazu wurden die Werte für die Variable $MMI$ berechnet: $HMMs >= 2.26$ und $LMMs <= .32$. Bezogen auf die Mediennutzung (siehe Tabelle \ref{table.deskrptMeediennutzung}) ergab sich zum Zeitpunkt der Befragung für Variable $MMI_{ext}$ eine Häufigkeitsverteilung von $LMMs = 10.5\%$ $(N=143)$ schwache Medien-Multitasker und $HMMs = 14.8\%$ $(N=202)$ starke Medien-Multitasker. 70.9\% ($N=969$) Personen befanden sich zwischen der Obergrenze $HMMs$ und der Untergrenze $LMMs$. 3.3\% ($N=45$) gaben an, kein Medien-Multitasking zu betreiben. Bezogen auf die Variable $MMI$ waren es $LMMs = 9.9\%$ $(N=136)$ schwache und $HMMs = 15.1\%$ $(N=207)$ starke Medien-Multitasker. 71\% ($N=971$) Personen befanden sich zwischen $HMMs$ und $LMMs$. 3.3\% ($N=45$) gaben an, kein Medien-Multitasking zu betreiben.

% UnterKapitel Aufmerksamkeitskontroll-Skala
%---------------------
\subsection{Aufmerksamkeitskontroll-Skala -- ACS.}
Die Aufmerksamkeitskontroll-Skala dient der Erfassung der allgemeinen Unterschiede der Probanden in der selbstinitierten Fähigkeit zur Aufmerksamkeitskontrolle (siehe dazu Kapitel \ref{subsection.acs} - \nameref{subsection.acs}). Für diese Variable füllten alle Probanden ($N=1367$) die Fragen zur Aufmerskamkeitskontrolle korrekt aus. Dabei entstand ein Mittelwert von $M = 53.43$, mit einer Standardabweichung von $SD = 6.82$. Das Minimum belief sich auf einen Wert von $Min = 34$, das Maximum auf $Max = 75$ (mögliches Minimum = 20 und mögliches Maximum = 80). Eine detaillierte Tabelle dieser Werte ist im Anhang \ref{anhang.hauefigkeitstabelle} zu finden.
...
Die Arbeitshypothese 2 lautete: Zwischen der Fähigkeit zur Aufmerksamkeitskontrolle und dem subjektiven Wohlbefinden besteht ein positiver Zusammenhang. Studenten mit hoher Fähigkeit zur Aufmerksamkeitskontrolle beeinflussen das eigene subjektive Wohlbefinden positiv. Zur Überprüfung der Hypothese wurde diese wie folgt operationalisiert:
...
%---------------------
\subsection{Verdeckte Korrelationen.}
Um verdeckte Korrelationen zwischen der Fähigkeit zur Aufmerksamkeitskontrolle und dem subjektiven Wohlbefinden zu ermitteln, wurde die Stichprobe anhand des Variable Geschlecht aufgeteilt:
\par
...
%---------------------
\subsection{Verdeckte Korrelationen.}
Um verdeckte Korrelationen zwischen der Fähigkeit zur Aufmerksamkeitskontrolle und dem Medien-Multitasking Verhalten zu ermitteln, wurde die Stichprobe anhand des Geschlechts, des Alters und ob die Probanden Kinder haben aufgeteilt.
